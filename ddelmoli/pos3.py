...
def normalize_as_logp(counts):
    ...
    log_total = np.log(sum(iter(counts.values())))
        ...
        for i, w in enumerate(sentence):
            if i == 0:
                ...
                for t_1 in self.tagset:
                    sum_terms = [(self.transition(t, t_1) + alpha[(i - 1, t)])
                                 for t in self.tagset]
                    alpha[(i, t_1)] = self.emission(t_1, w) + logsumexp(sum_terms)
                ...
                for t_1 in self.tagset:
                    ...
                    for t in self.tagset:
                        ...
                        if term > max_term:
                            max_term = term
                            max_tag = t
                    delta[(i, t_1)] = self.emission(t_1, w) + max_term
                    bp[(i, t_1)] = max_tag
        #### END(YOUR CODE) ####
